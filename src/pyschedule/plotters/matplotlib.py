...
def plot(scenario,img_filename=None,resource_height=1.0,show_task_labels=True,
         color_prec_groups=False,hide_tasks=None,hide_resources=None,task_colors=dict(),fig_size=(15,5),
         vertical_text=False) :
    """
    ...
    try :
        ...
        print('Plotter backend not avaiable: matplotlib not installed')
        return False
    import random

    if hide_tasks is None:
        hide_tasks = []
    if hide_resources is None:
        hide_resources = []
    ...
    tasks = [ T for T in scenario.tasks() if T not in hide_tasks ]
    ...
    if color_prec_groups :
        edges += [ (T,T_) for P in set(scenario.precs_lax()) | set(scenario.precs_tight()) \
    ...
    colors += [ [ random.random() for i in range(3) ] for x in range(len(scenario.tasks())) ] #random colors
    ...
    visible_resources = [ R for R in scenario.resources() if R not in hide_resources ]
