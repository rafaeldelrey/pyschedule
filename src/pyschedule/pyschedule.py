...
class Scenario(_SchedElement):
    ...
    def set_timeinfo_from_start_and_end_time_and_steptime(self, start_time, end_time, delta_t):
        ...
        self.duration = end_time - start_time  # should be 24 hours
        self.horizon = int(self.duration / self.step_time)  # then horizon should be equal to 24
        print("Start time:", self.start_time)
        print("End time:", self.end_time)
        print("Horizon (number of time increments):", self.horizon)
        print("Step time:", self.steptime)
...
class Task(_SchedElement) :
    ...
    def __iadd__(self,other):
        if _isiterable(other):
            ...
            self.add_resources_req(other)
            return self
        elif isinstance(other, (Task,_TaskAffine)):
...
class _Slice(_SchedElement):
    ...
    def __getitem__(self,key):
        if isinstance(key,str):
            ...
            return AssertionError("Unhandled key type")
    ...
    def __str__(self):
        ...
        if self._start is not None or self._end is not None:
            ...
            if self._start is not None:
                slice_ += str(self._start)
            slice_ += ':'
            if self._end is not None: #large number
                slice_ += str(self._end)
            slice_ += ']'
        operator = ''
        if self.kind == 'diff':
            operator = '.diff'
        ...
        s = '%s%s%s%s' % (str(self.resource),param_str,slice_,operator)
        return s
